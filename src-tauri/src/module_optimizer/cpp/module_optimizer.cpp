...
std::atomic<uint64_t> ModuleOptimizerCpp::processed_combinations{0};
std::atomic<uint64_t> ModuleOptimizerCpp::total_combinations_count{0};

std::pair<uint64_t, uint64_t> ModuleOptimizerCpp::GetProgress() {
    return {processed_combinations.load(), total_combinations_count.load()};
}

void ModuleOptimizerCpp::ResetProgress() {
    processed_combinations.store(0);
    total_combinations_count.store(0);
}

void ModuleOptimizerCpp::IncrementProgress(uint64_t delta) {
    processed_combinations.fetch_add(delta);
}
...
std::vector<CompactSolution> ModuleOptimizerCpp::ProcessCombinationRange(
    ...
    processed_combinations.fetch_add(range_size);
...
std::vector<ModuleSolution> ModuleOptimizerCpp::StrategyEnumeration(
    ...
    processed_combinations.store(0);
    total_combinations_count.store(total_combinations);
...
std::vector<ModuleSolution> ModuleOptimizerCpp::StrategyEnumerationCUDA(
    ...
    if (TestCuda()) {
        ...
        processed_combinations.store(total_combinations);
